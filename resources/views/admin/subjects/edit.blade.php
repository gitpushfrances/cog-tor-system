            ...
            <form action="{{ route('admin.subjects.update', $subject) }}" method="POST">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="code" value="{{ old('code', $subject->code) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                        @error('code')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Subject Name</label>
                        <input type="text" name="name" value="{{ old('name', $subject->name) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                    ...
                    <div>
                        ...
                        <input type="number" name="units" value="{{ old('units', $subject->units) }}" min="1" max="10" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                ...
                <div class="mt-6 flex gap-3">
                    <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">Update Subject</button>
