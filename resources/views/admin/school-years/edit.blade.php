<x-app-layout>
    <x-slot name="header">
        ...
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit School Year</h2>
    </x-slot>
            ...
            <form action="{{ route('admin.school-years.update', $schoolYear) }}" method="POST">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="year_start" value="{{ old('year_start', $schoolYear->year_start) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                    ...
                    <div>
                        ...
                        <input type="number" name="year_end" value="{{ old('year_end', $schoolYear->year_end) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                        @error('year_end')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div class="col-span-2">
                        <label class="block text-sm font-medium text-gray-700">Status</label>
                        <select name="status" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            <option value="inactive" {{ old('status', $schoolYear->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
